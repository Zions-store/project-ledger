...
7. Validate generated documents for sensitive values, unresolved output markers, and managed-marker
   order before atomic writes. In a development checkout, run
   `python project-docs/tests/validate_output.py <path> --kind state|devlog|agents`; otherwise perform
   the same checks explicitly. Report the template, output language, and any omitted facts.
